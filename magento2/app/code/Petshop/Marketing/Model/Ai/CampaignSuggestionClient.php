...
namespace Petshop\Marketing\Model\Ai;
...
class CampaignSuggestionClient
{
    ...
    public function suggestCampaigns(array $insights, array $editorContext = []): array
    {
        ...
        if ($apiKey === '') {
            throw new LocalizedException(__('AI API key is missing in Petshop Marketing configuration.'));
        }
    ...
    private function requestChat(string $apiKey, array $messages): array
    {
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            try {
                ...
                if (!is_array($decoded)) {
                    $this->logger->error('Petshop Marketing AI response is not valid JSON', ['body' => mb_substr($body, 0, 2000)]);
