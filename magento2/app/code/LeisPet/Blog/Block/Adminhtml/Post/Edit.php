<?php
namespace LeisPet\Blog\Block\Adminhtml\Post;
...
class Edit extends Container
{
    ...
    protected function _construct()
    {
        ...
        if ($this->blogConfig->isAiEnabled()) {
            $onclick = 'if (window.leispetBlogAiOpen) { window.leispetBlogAiOpen(); } return false;';
    ...
    {
        if ($this->coreRegistry->registry('leispet_blog_post')?->getId()) {
            return __('Edit Post: %1', $this->escapeHtml($this->coreRegistry->registry('leispet_blog_post')->getTitle()));
        }
