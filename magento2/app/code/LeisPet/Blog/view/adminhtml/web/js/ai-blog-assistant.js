define([
    'jquery',
    'mage/translate',
    'LeisPet_Blog/js/components/reusable-modal'
    ...
    function getJobStorageKey(runtime) {
        return 'leispet_blog_ai_job_' + String(runtime.postId || 'new');
    }
    ...
    return function initAiAssistant(config, element) {
        ...
        const $status = $('#leispet-ai-status');
        let pollTimer = null;
        ...
        const setBadgeVisible = function (field) {
            const $badge = $('#leispet-ai-badge-' + field);
            if ($badge.length) {
        ...
        const openAssistantModal = function () {
            const modal = createReusableModal({
                ...
                modalClass: 'leispet-ai-assistant-modal',
                buttons: [
                    {
                        text: $t('Close'),
                ...
                const renderTable = function () {
                    ...
                    const rows = sorted
                        .map(function (topic) {
                            ...
                            return (
                                '<tr class="leispet-ai-topic-row">' +
                                    '<td class="leispet-ai-topic-cell leispet-ai-topic-cell-select">' +
                                        '<input type="radio" name="leispet-ai-topic" value="' +
                                        escapedTitle +
                                        '" data-pet-type="' +
                                        escapedPetType +
                                        '" data-tone="' +
                                        ...
                                        '/>' +
                                    '</td>' +
                                    '<td class="leispet-ai-topic-cell leispet-ai-topic-title">' + escapedTitle + '</td>' +
                                    '<td class="leispet-ai-topic-cell">' + escapedPetType + '</td>' +
                                    '<td class="leispet-ai-topic-cell">' + escapedTone + '</td>' +
                                    '<td class="leispet-ai-topic-cell leispet-ai-topic-reason">' + escapedReason + '</td>' +
                                '</tr>'
                            );
                        })
                        .join('');
                    ...
                    const bodyRows =
                        rows ||
                        '<tr><td class="leispet-ai-topic-cell" colspan="5">' +
                    ...
                    modal.setContent(
                        '<div>' +
                            ...
                            '<table class="leispet-ai-topic-table">' +
                                '<thead>' +
                                    '<tr>' +
                                        '<th>' + $t('Use') + '</th>' +
                                        '<th><button type="button" class="leispet-ai-sort" data-sort-field="title">' + $t('Topic') + ' ' + sortIndicator('title') + '</button></th>' +
                                        '<th><button type="button" class="leispet-ai-sort" data-sort-field="petType">' + $t('Pet Type') + ' ' + sortIndicator('petType') + '</button></th>' +
                                        '<th><button type="button" class="leispet-ai-sort" data-sort-field="tone">' + $t('Tone') + ' ' + sortIndicator('tone') + '</button></th>' +
                    ...
                    const $first = $modalElement.find('input[name="leispet-ai-topic"]:first');
        ...
        window.leispetBlogAiOpen = openAssistantModal;
        ...
        $(document).on('click', '.leispet-ai-field-action', function (event) {
