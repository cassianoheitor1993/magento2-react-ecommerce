define([
    'jquery',
    'LeisPet_Homepage/js/widget-config-editor',
    ...
    return function (config, element) {
        ...
        var $modal = $('#leispet-create-widget-modal');
        var $form = $('#leispet-create-widget-form');
        var $schedule = $('#leispet_schedule_fields');
        var $configJson = $('#leispet_config_json');
        var $widgetId = $('#leispet_widget_id');
        var $configEditorWrapper = $('#leispet_config_editor_wrapper');
        var $configEditorContainer = $('#leispet_config_editor');
        var $errorBox = $('#leispet-widget-modal-errors');
        var $errorContent = $('#leispet-widget-modal-errors-content');
        var $successBox = $('#leispet-widget-modal-success');
        var $scheduleHint = $('#leispet-schedule-validation-hint');
            ...
            modal({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        class: 'action-primary leispet-modal-save-btn',
                        click: function () {
                            handleSave(this);
                        }
                    }
        ...
        function openCreateModal() {
            ...
            $('#leispet_widget_type').prop('disabled', false);
        ...
        function populateFormFromWidget(widget) {
            ...
            // Title
            $('#leispet_widget_title').val(widget.title || '');

            // Widget type (set and disable to prevent accidental change)
            var $typeSelect = $('#leispet_widget_type');
            ...
            // Sort order
            $('#leispet_sort_order').val(widget.sort_order || 0);
            ...
            configEditor.load($configEditorContainer, widget.widget_type, parsedData, function () {
                ...
                $('#leispet_starts_at').val(formatDateForInput(widget.starts_at || ''));
                $('#leispet_ends_at').val(formatDateForInput(widget.ends_at || ''));

                // Set active status
                $('#leispet_is_active').val(String(widget.is_active || 0));
        ...
        function resetForm() {
            ...
            $widgetId.val('');
            $configJson.val('');
            $('#leispet_widget_title').val('');
            $('#leispet_sort_order').val('0');
        ...
        function initializeDateTimePickers() {
            var $start = $('#leispet_starts_at');
            var $end = $('#leispet_ends_at');

            [$start, $end].forEach(function ($input) {
                if (!$input.length || $input.data('leispetCalendarInit')) {
                    return;
                }
                ...
                $input.data('leispetCalendarInit', true);
            });
        }
        ...
        function updateScheduleVisibility() {
            var widgetType = $.trim(String($('#leispet_widget_type').val() || ''));
        ...
        function runScheduleValidation() {
            ...
            var startsAt = $.trim(String($('#leispet_starts_at').val() || ''));
            var endsAt = $.trim(String($('#leispet_ends_at').val() || ''));
            var isActive = String($('#leispet_is_active').val() || '1');
            var widgetType = $.trim(String($('#leispet_widget_type').val() || ''));
        ...
        // Dismiss errors
        $(document).on('click', '#leispet-widget-modal-errors-close', function () {
            $errorBox.hide();
            $errorContent.empty();
        });

        // Generate content with AI
        $(document).on('click', '#leispet_generate_widget_content', function () {
            var widgetType = $.trim(String($('#leispet_widget_type').val() || ''));
            var contextText = $.trim(String($('#leispet_widget_context').val() || ''));
            var $button = $(this);
        ...
        // Widget type change
        $(document).on('change', '#leispet_widget_type', function () {
        ...
        $(document).on('change input', '#leispet_starts_at, #leispet_ends_at, #leispet_is_active', function () {
            scheduleValidationDebounced();
        });

        // ── "Add Widget" button → Create mode ────────────────────────

        $(document).on('click', '#leispet-open-create-widget-modal', function () {
            openCreateModal();
        });
