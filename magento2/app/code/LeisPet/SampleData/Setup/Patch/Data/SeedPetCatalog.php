    ...
    private function seedCatalog(): void
    {
        ...
        foreach ($petTypes as $petType) {
            foreach ($lines as $index => $line) {
                $count++;
                $sku = sprintf('leispet-%s-%02d', strtolower($petType['name']), $count);
                ...
                $description = sprintf(
                    '%s %s designed for daily nutrition and freshness. Balanced ingredients, pet-safe packaging, and quality-tested by LeisPet.',
                    $petType['name'],
                    $line['title']
                );
