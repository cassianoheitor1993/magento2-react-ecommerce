...
module.exports = async env => {
    ...
    config.plugins = [
        ...
        new DefinePlugin({
            ...
            ENABLE_ADOBE_EVENTS: JSON.stringify(
                process.env.ENABLE_ADOBE_EVENTS || 'true'
            ),
            ENABLE_STOREFRONT_REVAMP: JSON.stringify(
                process.env.ENABLE_STOREFRONT_REVAMP || 'true'
            ),
            ENABLE_SIGNED_IN_PERSONALIZATION: JSON.stringify(
                process.env.ENABLE_SIGNED_IN_PERSONALIZATION || 'false'
            ),
            SIGNED_IN_PERSONALIZATION_ROLLOUT: JSON.stringify(
                process.env.SIGNED_IN_PERSONALIZATION_ROLLOUT || '0'
            ),
            ADOBE_EVENT_SAMPLE_RATE: JSON.stringify(
                process.env.ADOBE_EVENT_SAMPLE_RATE || '1'
            ),
            ENABLE_KPI_PROBES: JSON.stringify(
                process.env.ENABLE_KPI_PROBES || 'true'
            ),
            ROLLBACK_DISABLE_SEARCH_TELEMETRY: JSON.stringify(
                process.env.ROLLBACK_DISABLE_SEARCH_TELEMETRY || 'false'
            ),
            ROLLBACK_DISABLE_JSON_LD: JSON.stringify(
                process.env.ROLLBACK_DISABLE_JSON_LD || 'false'
            ),
            ROLLBACK_DISABLE_SOCIAL_PROOF: JSON.stringify(
                process.env.ROLLBACK_DISABLE_SOCIAL_PROOF || 'false'
            ),
